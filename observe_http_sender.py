...
class ObserveHttpSender:
   ...
   Arguments:
    ...
    def __init__(self, obsv_url, token):
        ...
        self.customer_id = re.search(
            url_regex_pattern, obsv_url).group("customer_id")
        self.observer_instance = re.search(
            url_regex_pattern, obsv_url).group("obsv_domain")
    ...
    def set_concurrent_post_limit(self, value=5):
        ...
        self.log.info(
            "Observer Post Max Concurrent limit: concurrent_post_limit={0}".format(value))
    ...
    def set_post_max_byte_size(self, value=4000):
        ...
        self.log.info(
            "Observer Post Max Byte Size: max_byte_size={0}".format(value))
            ...
            if response_status_code == 200:
                ...
                if response_status_code in ACCEPTABLE_STATUS_CODES:
                    ...
                    self.log.error("HTTP status_code=%s message=%s Customer=%s Instance=%s",
                                   response_status_code, response_text, self.customer_id, self.observer_instance)
        except Exception as e:
    ...
    def post_observation(self, payload):
        ...
        if self._payload_mode_json and type(payload) is not dict:
            self.log.warn("Ignoring Payload: set_payload_json_format:%s expected type dict received type %s",
                          self._payload_mode_json, str(type(payload)))
            return
        elif not self._payload_mode_json and type(payload) is not str:
            self.log.warn("Ignoring Payload: set_payload_json_format:%s expected type str received type %s",
                          self._payload_mode_json, str(type(payload)))
            return
